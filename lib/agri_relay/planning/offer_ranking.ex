defmodule AgriRelay.Planning.OfferRanking do
  @moduledoc "Deterministic offer ranking evaluation for auditable farm planning."
  def evaluate(%{observed: observed, baseline: baseline} = input)
    ...
    with :ok <- validate_ratio(confidence), true <- is_number(threshold) and threshold >= 0 do
      ...
      {:ok, %{score: score, triggered: score >= threshold, reason: "offer ranking"}}
    else
