defmodule AgriRelay.Planning.PickupPriority do
  @moduledoc "Deterministic pickup priority evaluation for auditable farm planning."
  def evaluate(%{observed: observed, baseline: baseline} = input)
    ...
    with :ok <- validate_ratio(confidence), true <- is_number(threshold) and threshold >= 0 do
      ...
      {:ok, %{score: score, triggered: score >= threshold, reason: "pickup priority"}}
    else
